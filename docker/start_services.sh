...
if command -v ollama > /dev/null 2>&1; then
    ...
    # Optional model pull on start - ollama pull skips models already present (no re-download)
    if [ "${OLLAMA_PULL_ON_START:-false}" = "true" ]; then
        (
            sleep 10  # Wait for Ollama to fully start
            echo "📥 Ensuring Ollama models (VLM: minicpm-v; SLM: qwen2.5:3b) - skips if already present..."
            for m in minicpm-v qwen2.5:3b; do ollama pull $m 2>&1 || true; done | tee /tmp/logs/ollama_pull.log
